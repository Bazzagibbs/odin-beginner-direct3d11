...
WINDOW_NAME :: "08. Using a High Precision Timer"
...
virtual_key_to_input_action :: proc "contextless" (vkey: win.WPARAM) -> Input_Action {
        switch vkey {
        case win.VK_W, win.VK_UP:
                return .Up
        case win.VK_S, win.VK_DOWN:
                return .Down
        case win.VK_A, win.VK_LEFT:
                return .Left
        case win.VK_D, win.VK_RIGHT:
                return .Right
        case:
                return .Unknown
        }
...
main :: proc() {
        ...
        triangle_pos       := [2]f32 {0, 0}
        triangle_move_rate : f32 = 0.5
        ...
        for is_running {
                ...
                if Input_Action.Up in inputs {
                        delta_move += {0, 1}
                }
                if Input_Action.Down in inputs {
                        delta_move += {0, -1}
                }
                ...
                delta_move = linalg.vector_normalize0(delta_move)
                triangle_pos += delta_move * (triangle_move_rate * delta_time)
