...
WINDOW_NAME :: "12. Blinn-Phong Shading"

assert_messagebox :: helpers.assert_messagebox

Matrix3_Padded    :: helpers.Matrix3_Padded
pad_matrix3       :: helpers.pad_matrix3
...
main :: proc() {
        ...
        index_count  : u32
        {      
                mesh_data, mesh_ok := wavefront_load(#load("suzanne.obj"))
                assert_messagebox(mesh_ok, "Load OBJ failed")
                defer wavefront_destroy(&mesh_data)

                // vertex 
                vertex_stride = size_of(Vertex_Data)
                vertex_count  = u32(len(mesh_data.vertex_buffer))
                vertex_offset = 0

                vertex_buffer_desc := d3d11.BUFFER_DESC {
                        ByteWidth = u32(slice.size(mesh_data.vertex_buffer[:])),
                        Usage     = .IMMUTABLE,
                        BindFlags = {.VERTEX_BUFFER},
                }

                vertex_subresource_data := d3d11.SUBRESOURCE_DATA { 
                        pSysMem = raw_data(mesh_data.vertex_buffer)
                }
                ...
                // index
                index_count = u32(len(mesh_data.index_buffer))

                index_buffer_desc := d3d11.BUFFER_DESC {
                        ByteWidth = u32(slice.size(mesh_data.index_buffer[:])),
                        Usage     = .IMMUTABLE,
                        BindFlags = {.INDEX_BUFFER},
                }

                index_subresource_data := d3d11.SUBRESOURCE_DATA {
                        pSysMem = raw_data(mesh_data.index_buffer)
                }
        ...
        Constants_Vertex :: struct #align(16) {
                model_view_projection  : matrix[4,4]f32,
                model_view             : matrix[4,4]f32,
                normal_matrix          : Matrix3_Padded,  // matrix[3,3]f32 doesn't have column padding
        }
        
        constant_buffer_vertex : ^d3d11.IBuffer
        {
                #assert(size_of(Constants_Vertex) % 16 == 0, "Constant buffer size must be a multiple of 16")

                buffer_desc := d3d11.BUFFER_DESC {
                        ByteWidth      = size_of(Constants_Vertex),
                        Usage          = .DYNAMIC,
                ...
                res := device->CreateBuffer(
                        pDesc = &buffer_desc, 
                        pInitialData = nil,
                        ppBuffer = &constant_buffer_vertex
                )
                assert_messagebox(res, "Create ConstantBuffer failed")
        }
        defer constant_buffer_vertex->Release()


        Directional_Light :: struct #packed {
                direction_to_light : [4]f32, // surface to light
                color              : [4]f32,
        }

        Point_Light :: struct #packed {
                position : [4]f32,
                color    : [4]f32,
        }

        Constants_Pixel :: struct #align(16) {
                directional_light : Directional_Light,
                point_lights      : [2]Point_Light,
        }

        
        constant_buffer_pixel : ^d3d11.IBuffer
        {
                #assert(size_of(Constants_Pixel) % 16 == 0, "Constant buffer size must be a multiple of 16")

                buffer_desc := d3d11.BUFFER_DESC {
                        ByteWidth      = size_of(Constants_Pixel),
                        Usage          = .DYNAMIC,
                        BindFlags      = {.CONSTANT_BUFFER},
                        CPUAccessFlags = {.WRITE},
                }
                ...
                res := device->CreateBuffer(
                        pDesc = &buffer_desc, 
                        pInitialData = nil,
                        ppBuffer = &constant_buffer_pixel
                )
                assert_messagebox(res, "Create ConstantBuffer failed")
        }
        defer constant_buffer_pixel->Release()


        // Game loop
        cam_position := [3]f32 {0, 0.75, 2}
        cam_rotation := [2]f32 {0.25, math.PI}
        ...
        point_lights := [2]Point_Light {
                {position = {5, 5, -2, 0},    color = {0.5, 0.7, 1, 1}},
                {position = {-5, 2, -3, 0}, color = {0.5, 0.3, 1, 1}},
        }

        directional_light := Directional_Light {
                direction_to_light = {0.5, 1, 1, 0}, color = {1, 0.8, 0.6, 1},
        }
        ...
        for is_running {
                ...
                // Mesh
                // vertex constants
                model_view := view_matrix * mesh_model_matrix
                normal_matrix := (matrix[3,3]f32)(linalg.inverse_transpose(model_view))

                mesh_constants_vertex := Constants_Vertex { 
                        model_view_projection = projection_matrix * model_view,
                        model_view            = model_view,
                        normal_matrix         = pad_matrix3(normal_matrix),
                }

                mapped_constant_buffer : d3d11.MAPPED_SUBRESOURCE
                device_context->Map(constant_buffer_vertex, 0, .WRITE_DISCARD, {}, &mapped_constant_buffer)
                mem.copy(mapped_constant_buffer.pData, &mesh_constants_vertex, size_of(Constants_Vertex))
                device_context->Unmap(constant_buffer_vertex, 0)

                // pixel constants
                directional_eye := linalg.normalize(view_matrix * directional_light.direction_to_light)
                mesh_constants_pixel := Constants_Pixel {
                        directional_light = {
                                direction_to_light = directional_eye,
                                color              = directional_light.color,
                        },
                        point_lights = {
                                {
                                        position = view_matrix * point_lights[0].position, 
                                        color = point_lights[0].color
                                }, 
                                {
                                        position = view_matrix * point_lights[1].position, 
                                        color = point_lights[1].color
                                },
                        }
                }
                
                device_context->Map(constant_buffer_pixel, 0, .WRITE_DISCARD, {}, &mapped_constant_buffer)
                mem.copy(mapped_constant_buffer.pData, &mesh_constants_pixel, size_of(Constants_Pixel))
                device_context->Unmap(constant_buffer_pixel, 0)


                device_context->VSSetConstantBuffers(0, 1, &constant_buffer_vertex)
                device_context->PSSetConstantBuffers(0, 1, &constant_buffer_pixel)
